...
var S8_ITEMS = {
  ...
  solar:    { glb:'sceen_5_bagian_2_energi_terbarukan_solar_panel_1.glb',  targetSize:4   },
  ...
  water2:   { glb:'sceen_5_bagian_3_pengelolaan_air_traktor_air_1.glb',    targetSize:5   },
  rock:     { glb:'batu.glb',                                              targetSize:2.5 },
  lion:     { glb:'singa.glb',                                             targetSize:3.5 },
};

// State
var s8_ticker=0, s8_animObjs=[], s8_sceneRef8=null, s8_particles8=[];
var s8_lights8=[], s8_placedKeys=[], s8_isBuilt=false;

// ============================================================
// BUILD SCENE 8
// ============================================================
function buildScene8(){
  s8_ticker=0; s8_animObjs=[]; s8_particles8=[]; s8_lights8=[];
  s8_placedKeys=[]; s8_sceneRef8=scene;
  ...
  scene.add(new THREE.DirectionalLight(0x4488ff,0.55)).position.set(-25,15,-12);
  ...
  if(nEl) nEl.textContent='🌍 Ini adalah hasil desain ekosistemmu! Tekan "Simulasi Waktu" untuk melihat dampak jangka panjang.';
...
function _s8_buildTerrain(){
  ...
  island.position.y=-1.9; scene.add(island);
  ...
  grass.position.y=0.25; scene.add(grass);
  ...
  innerRing.position.y=0.54; scene.add(innerRing);
  ...
  scene.add(function(){
    ...
    rb.rotation.x=-Math.PI/2; rb.position.set(0.5,-0.02,0); return rb;
  }());
  ...
  water.rotation.x=-Math.PI/2; water.position.set(0.5,0.06,0);
  water.userData.waterSurface=true; scene.add(water); s8_animObjs.push(water);
  ...
  for(var cl=0;cl<20;cl++){
    ...
    cliff.position.set(Math.cos(ca)*cr,-2.2-Math.random()*1.2,Math.sin(ca)*cr);
    cliff.rotation.y=ca; scene.add(cliff);
  }

  // Kabut bawah pulau
  for(var cf=0;cf<10;cf++){
    var cfang=Math.random()*Math.PI*2,cfr=5+Math.random()*13;
    var cloud=new THREE.Mesh(
      new THREE.SphereGeometry(2+Math.random()*3,8,6),
      new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:0.025+Math.random()*0.04})
    );
    ...
    scene.add(cloud); s8_animObjs.push(cloud);
  }
  ...
  for(var og=0;og<3;og++){
    var outerR=new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({color:[0x00e676,0x42a5f5,0xffca28][og],transparent:true,opacity:0.12-og*0.03})
    );
    outerR.rotation.x=Math.PI/2;
    outerR.position.y=0.12+og*0.04;
    outerR.userData.rotSpeed=(og%2===0?0.05:-0.035);
    scene.add(outerR); s8_animObjs.push(outerR);
  }

  // Matahari
  var sunball=new THREE.Mesh(new THREE.SphereGeometry(5,20,20),new THREE.MeshBasicMaterial({color:0xfff5a0,transparent:true,opacity:0.88}));
  sunball.position.set(55,35,-70); scene.add(sunball);
  var halo=new THREE.Mesh(new THREE.SphereGeometry(8,16,16),new THREE.MeshBasicMaterial({color:0xffec40,transparent:true,opacity:0.18}));
  halo.position.copy(sunball.position); scene.add(halo);
  ...
  groundPlane.rotation.x=-Math.PI/2; groundPlane.position.y=-8; scene.add(groundPlane);
  var bigGrid=new THREE.GridHelper(200,40,0x003322,0x001a10);
  bigGrid.position.y=-7.9; bigGrid.material.transparent=true; bigGrid.material.opacity=0.5; scene.add(bigGrid);

  // 8 menara sci-fi
  [0,45,90,135,180,225,270,315].forEach(function(deg,ti){
    var rad=deg*Math.PI/180, tDist=38+Math.sin(ti*1.3)*4;
    var tx=Math.cos(rad)*tDist, tz=Math.sin(rad)*tDist;
    var tH=18+Math.sin(ti*0.8)*8;
    var shaft=new THREE.Mesh(new THREE.CylinderGeometry(0.35,0.55,tH,8),
      new THREE.MeshPhongMaterial({color:0x0d2218,emissive:new THREE.Color(0x003322),emissiveIntensity:0.3}));
    shaft.position.set(tx,-7.2+tH/2,tz); scene.add(shaft);
    var pedestal=new THREE.Mesh(new THREE.CylinderGeometry(1.8,2.4,1.5,8),
      new THREE.MeshPhongMaterial({color:0x0a1a14,emissive:new THREE.Color(0x003322),emissiveIntensity:0.4}));
    pedestal.position.set(tx,-7.2,tz); scene.add(pedestal);
    var crystalTop=new THREE.Mesh(new THREE.OctahedronGeometry(0.9,0),
    ...
    crystalTop.userData.crystalPulse=true; crystalTop.userData.crystalPhase=(ti/8)*Math.PI*2;
    scene.add(crystalTop); s8_animObjs.push(crystalTop);
    var auraRing=new THREE.Mesh(new THREE.TorusGeometry(1.4,0.06,6,20),
      new THREE.MeshBasicMaterial({color:ti%3===0?0x00e676:ti%3===1?0x42a5f5:0xffca28,transparent:true,opacity:0.35}));
    auraRing.rotation.x=Math.PI/2; auraRing.position.set(tx,-7.2+tH+0.5,tz);
    auraRing.userData.rotSpeed=(ti%2===0?1.5:-1.2); scene.add(auraRing); s8_animObjs.push(auraRing);
    ...
    towerLight.userData.baseInt=0.7; towerLight.userData.pulseOff=(ti/8)*Math.PI*2;
    scene.add(towerLight); s8_lights8.push(towerLight);
  });
}

// ============================================================
// RESTORE SCENE-7 DESIGN — load placed GLB items
// ============================================================
function _s8_restoreDesign(){
  var raw=null;
  try{ raw=localStorage.getItem('ar_s7_placed'); }catch(e){}

  if(!raw || raw==='[]' || raw==='null'){
    // Desain kosong — tampilkan hint
    _s8_showEmptyHint();
    return;
  }
  ...
  if(!keys||!keys.length){ _s8_showEmptyHint(); return; }

  s8_placedKeys=keys;

  // Re-use slot system dari scene-7 (mirror logic)
  var s8SlotIdx={};
  var s8OccupiedPos=[];
  ...
  var S8_RADIUS={tree:2.8,forest:5.5,water:3.2,water2:4,solar:2.5,animal:3,lion:3.5,recycle:3,building:3.5,rock:1.8};
  for(var k in S8_SLOTS) s8SlotIdx[k]=0;

  function isTooClose(x,z,r){
    for(var i=0;i<s8OccupiedPos.length;i++){
      var op=s8OccupiedPos[i];
      if(Math.sqrt(Math.pow(x-op.x,2)+Math.pow(z-op.z,2))<r+(op.r||2.5)) return true;
    }
    return false;
  }
  function insideBounds(x,z,r){ return Math.sqrt(x*x+z*z)+r<=S8_ISLAND_R; }
  function findSlot(key){
    ...
    while(tried<slots.length){
      ...
      if(insideBounds(sx,sz,radius)&&!isTooClose(sx,sz,radius)){
        s8SlotIdx[sk]=si+1;
        return{x:sx,z:sz};
      }
      idx++;tried++;
    ...
    for(var sp=1;sp<25;sp++){
      var ang=sp*2.4, dist=Math.min(sp*1.5,S8_ISLAND_R-radius-1);
      var fx=base[0]+Math.cos(ang)*dist, fz=base[2]+Math.sin(ang)*dist;
      var posR=Math.sqrt(fx*fx+fz*fz);
      if(posR+radius>S8_ISLAND_R){ var sc=(S8_ISLAND_R-radius-0.5)/posR; fx*=sc; fz*=sc; }
      if(!isTooClose(fx,fz,radius)) return{x:fx,z:fz};
    }
    return null;
  }
...
function _s8_showEmptyHint(){
  ...
  hint.style.cssText='position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);z-index:80;'+
  ...
  styleEl.textContent='@keyframes s8hintIn{from{opacity:0;transform:translate(-50%,-50%) scale(0.85)}to{opacity:1;transform:translate(-50%,-50%) scale(1)}}';
...
function _s8_spawnAtmosphere(){
  ...
  for(var i=0;i<50;i++){
    ...
    var p=new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({color:c,transparent:true,opacity:0.2+Math.random()*0.3})
    );
    p.position.set((Math.random()-0.5)*50,0.5+Math.random()*9,(Math.random()-0.5)*32);
    ...
    p.userData.pBaseX=p.position.x; p.userData.pBaseZ=p.position.z;
    p.userData.isAtmos=true;
    scene.add(p); s8_particles8.push(p);
  }
}
...
function _s8_startLoop(){
  var lastT=performance.now(); var ref=s8_sceneRef8;
  function tick(now){
    if(!scene||scene!==ref) return;
    var dt=Math.min((now-lastT)/1000,0.1); lastT=now; s8_ticker+=dt; var t=s8_ticker;

    s8_animObjs.forEach(function(obj){
      if(!obj.parent) return;
      if(obj.userData.waterSurface){ obj.material.opacity=0.78+Math.sin(t*1.8)*0.05; return; }
      if(obj.userData.rotSpeed!==undefined){ obj.rotation.z+=obj.userData.rotSpeed*dt; return; }
      if(obj.userData.crystalPulse){
        var sc=0.85+Math.sin(t*3+obj.userData.crystalPhase)*0.2;
        obj.scale.setScalar(sc); obj.rotation.y+=1.5*dt;
        obj.material.opacity=0.5+Math.sin(t*3+obj.userData.crystalPhase)*0.35; return;
      }
      if(obj.userData.cloudBase){
        obj.position.x=obj.userData.cloudBase.x+Math.sin(t*0.2+obj.userData.cloudPhase)*1.5; return;
      }
      if(obj.userData.s8Rising){
        ...
        if(Math.abs(obj.position.y-obj.userData.s8TargetY)<0.02){
          obj.position.y=obj.userData.s8TargetY; obj.userData.s8Rising=false;
        }
        return;
      }
      if(obj.userData.s8Float!==undefined&&!obj.userData.s8Rising){
        obj.position.y=(obj.userData.s8BaseY||0)+Math.sin(t*obj.userData.s8Float+obj.userData.s8FloatPhase)*0.07;
      }
    });
    ...
    for(var j=0;j<s8_particles8.length;j++){
      var pt=s8_particles8[j]; if(!pt.parent) continue;
      if(pt.userData.isAtmos){
        pt.position.y+=pt.userData.pvy; pt.position.x+=pt.userData.pvx; pt.position.z+=pt.userData.pvz;
...
function _s8_injectUI(){
  ...
  var counts={};
  s8_placedKeys.forEach(function(k){ counts[k]=(counts[k]||0)+1; });
  ...
  panel.innerHTML=
    ...
    '<div class="s8-stat-row"><span class="s8-stat-lbl">🌿 Komponen Flora</span><span class="s8-stat-val">'+flora+'</span></div>'+
    '<div class="s8-stat-row"><span class="s8-stat-lbl">🦌 Komponen Fauna</span><span class="s8-stat-val">'+fauna+'</span></div>'+
    '<div class="s8-stat-row"><span class="s8-stat-lbl">⚡ Energi Bersih</span><span class="s8-stat-val">'+energi+'</span></div>'+
    '<div class="s8-stat-row"><span class="s8-stat-lbl">💧 Sumber Air</span><span class="s8-stat-val">'+air+'</span></div>'+
    '<div class="s8-stat-row"><span class="s8-stat-lbl">🏡 Urban Hijau</span><span class="s8-stat-val">'+urban+'</span></div>'+
    '<div class="s8-stat-row" style="padding-top:6px;border-top:1px solid rgba(255,255,255,0.07)"><span class="s8-stat-lbl">🧩 Total Komponen</span><span class="s8-stat-val">'+total+'</span></div>'+
  ...
  modal.innerHTML='<div id="s8-result-card">'+
...
window.s8_runSimulation=function(){
  ...
  var score=0;
  if(flora>0) score+=20; if(fauna>0) score+=20;
  if(energi>0) score+=20; if(air>0) score+=20; if(urban>0) score+=20;
  // Bonus keragaman
  if(flora>=2) score+=5; if(fauna>=2) score+=5;
  if(energi>=2) score+=5; if(air>=2) score+=5;
  score=Math.min(score,100);

  var icon,title,body;
  if(total===0){
    icon='❌'; title='Ekosistem Kosong';
    body='Tidak ada komponen yang ditambahkan. Kembali ke Scene 7 dan desain ekosistemmu!';
  } else if(score>=90){
    icon='🏆'; title='Ekosistem Sempurna!';
    body='<b>Hari 1 → Tahun 5:</b><br>'+
    ...
    icon='⚠️'; title='Ekosistem Hampir Seimbang';
    body='<b>Hari 1 → Tahun 5:</b><br>'+
      (flora>0?'✅':'❌')+' Flora: '+(flora>0?'Berkembang baik':'Perlu ditambah')+'\n'+
      (fauna>0?'✅':'❌')+' Fauna: '+(fauna>0?'Populasi stabil':'Tidak ada satwa liar')+'\n'+
      (energi>0?'✅':'❌')+' Energi: '+(energi>0?'Emisi berkurang':'Masih bergantung energi fosil')+'\n'+
      (air>0?'✅':'❌')+' Air: '+(air>0?'Sumber air terjaga':'Kekurangan air bersih')+'\n\n'+
      '<b>💡 Tambahkan komponen yang masih kurang untuk ekosistem sempurna!</b>';
  } else {
    icon='❌'; title='Ekosistem Tidak Seimbang';
    body='<b>Hari 1 → Tahun 5:</b><br>'+
      '❌ Keanekaragaman hayati rendah ('+total+' komponen saja)<br>'+
...
window.addEventListener('DOMContentLoaded',function(){ startSceneApp(7); });
