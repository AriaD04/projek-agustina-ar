...
var S7_ITEMS = [
  {
    ...
    color: "#2d9e50",
    category: "flora",
    desc: "Penghasil O₂ & habitat makhluk hidup",
  },
  {
    key: "forest",
    icon: "🌲",
    label: "Hutan Lebat",
    glb: "low_poly_tree_scene_free-compressed.glb",
    targetSize: 9,
    xp: 120,
    color: "#00e676",
    category: "flora",
    desc: "Hutan lebat meningkatkan biodiversitas & penyerapan karbon",
  },
  {
    key: "water",
    icon: "💧",
    label: "Kolam Air",
    glb: "genangan_air.glb",
    targetSize: 4,
    xp: 60,
    color: "#42a5f5",
    category: "air",
    desc: "Sumber air bersih untuk ekosistem",
  },
  {
    key: "solar",
    icon: "⚡",
    ...
    color: "#ffca28",
    category: "energi",
    desc: "Energi bersih nol emisi karbon",
  },
  {
    key: "animal",
    icon: "🦌",
    ...
    color: "#ff7043",
    category: "fauna",
    desc: "Rusa herbivora penjaga keseimbangan rantai makanan",
  },
  {
    key: "recycle",
    icon: "♻️",
    label: "Daur Ulang",
    glb: "sceen_5_bagian_1_recycle_1.glb",
    targetSize: 4.5,
    xp: 70,
    color: "#26c6da",
    category: "urban",
    desc: "Pengolahan sampah menjadi sumber daya baru",
  },
  {
    key: "building",
    icon: "🏡",
    label: "Bangunan Hijau",
    glb: "sceen_5_bagian_4_ruang_hijau_kota_1.glb",
    targetSize: 5,
    xp: 75,
    color: "#558b2f",
    category: "urban",
    desc: "Ruang hijau kota kurangi heat island",
  },
  {
    key: "water2",
    icon: "🚿",
    ...
    color: "#29b6f6",
    category: "air",
    desc: "Irigasi cerdas & konservasi air bersih",
  },
  {
    key: "rock",
    icon: "🪨",
    ...
    color: "#90a4ae",
    category: "flora",
    desc: "Elemen abiotik penyeimbang ekosistem",
  },
  {
    key: "lion",
    icon: "🦁",
    ...
    color: "#ffa726",
    category: "fauna",
    desc: "Predator puncak penjaga keseimbangan populasi",
  },
];
...
function buildScene7() {
  ...
  scene.fog = new THREE.FogExp2(0x030a05, 0.007); // density dikurangi agar island tidak tertutup kabut
  ...
  sun.castShadow = true;
  scene.add(sun);
  var fill = new THREE.DirectionalLight(0x4488ff, 0.55);
  fill.position.set(-25, 15, -12);
  scene.add(fill);
  ...
  // ── Selalu bersih saat pertama buka ──
  try {
    localStorage.removeItem("ar_s7_placed");
    localStorage.removeItem("ar_s7_placed_full");
    localStorage.removeItem("ar_s7_xp");
    localStorage.removeItem("ar_builtObjects");
  } catch (e) {}
  s7_baseXP = 0;
  s7_totalXP = 0;

  // ── FIX: REVISI TOTAL POSISI KAMERA (Sudut Serong Diagonal) ──
  if (typeof orbit !== "undefined" && orbit) {
    orbit.radius = 10; // Jarak kamera mundur agar seluruh pulau masuk frame tanpa terpotong
    orbit.phi = 1; // Sudut kemiringan vertikal (membuat kamera turun melihat secara miring)
    orbit.theta = 49; // Rotasi horizontal (memutar sudut pandang 45 derajat dari sudut pojok pulau)
    orbit.tx = 0;
    orbit.ty = 50; // Menaikkan sedikit target fokus kamera agar pas di atas permukaan objek pulau
    orbit.tz = 0;
    if (typeof applyOrbit === "function") {
      applyOrbit();
    }
  }

  // Penguncian langsung pada objek kamera utama Three.js untuk mencegah overwrite dari script luar
  if (typeof camera !== "undefined" && camera) {
    camera.position.set(28, 22, 28); // Koordinat X, Y, Z untuk tampilan sudut pandang isometrik nyata
    camera.lookAt(0, -6, 0); // Mengarahkan pandangan tepat ke tengah pulau
  }

  // Desain selalu mulai bersih (localStorage sudah dihapus di atas)
  _s7_injectUI();

  var nEl = document.getElementById("narasi-text");
  if (nEl)
    nEl.textContent =
      "🏗️ Mode Builder aktif! Pilih komponen di panel kiri — objek akan muncul di zona yang tepat.";
...
function _s7_buildEpicTerrain() {
  // Terrain group diturunkan ke Y=-10 agar pulau berada tepat di depan kamera
  var TERRAIN_Y = -10;
  var root = new THREE.Group();
  root.position.y = TERRAIN_Y;
  scene.add(root);
  ...
  root.add(halo);
  scene
    .add(new THREE.PointLight(0xfff5a0, 1.0, 220))
    .position.copy(sunball.position);
  ...
  for (var gr = 0; gr < 4; gr++) {
    var floorRing = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({
        ...
        transparent: true,
        opacity: 0.08 - gr * 0.015,
      }),
    );
    floorRing.rotation.x = Math.PI / 2;
    floorRing.position.y = -7.8;
    floorRing.userData.rotSpeed =
      (gr % 2 === 0 ? 0.03 : -0.022) * (1 + gr * 0.3);
    root.add(floorRing);
  ...
  towerAngles.forEach(function (deg, ti) {
    ...
    for (var rr = 0; rr < ringCount; rr++) {
      var towerRing = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({
          color: ti % 2 === 0 ? 0x00e676 : 0x00bcd4,
          transparent: true,
          opacity: 0.5 + rr * 0.05,
        }),
      );
      ...
      towerRing.userData.rotSpeed =
        (rr % 2 === 0 ? 1.2 : -0.9) * (0.5 + rr * 0.1);
      root.add(towerRing);
  ...
  var hexPositions = [
    { x: -55, z: -45, y: -5, r: 10, h: 1.2 },
    { x: 58, z: -42, y: -4, r: 9, h: 1.0 },
    { x: -52, z: 38, y: -6, r: 11, h: 1.4 },
    { x: 55, z: 40, y: -5, r: 8, h: 1.0 },
    { x: 0, z: -65, y: -3, r: 13, h: 1.5 },
    { x: 0, z: 65, y: -4, r: 12, h: 1.3 },
  ];
  hexPositions.forEach(function (hp, hi) {
    var hexPlat = new THREE.Mesh(
      ...
      new THREE.MeshPhongMaterial({
        color: 0x0a1a12,
        emissive: new THREE.Color(0x001a0e),
    ...
    var hexRing = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({
        color: [0x00e676, 0x42a5f5, 0xffca28, 0xff6d00, 0x00bcd4, 0xce93d8][hi],
        transparent: true,
        opacity: 0.55,
      }),
    );
    ...
    var hexGrid = new THREE.GridHelper(
      hp.r * 1.8,
      5,
      [0x003322, 0x001a33][hi % 2],
      [0x002211, 0x00112a][hi % 2],
    );
    hexGrid.position.set(hp.x, hp.y + hp.h / 2 + 0.06, hp.z);
    hexGrid.material.transparent = true;
    hexGrid.material.opacity = 0.4;
    root.add(hexGrid);
    ...
    for (var mt = 0; mt < 3; mt++) {
      ...
      miniTower.position.set(
        hp.x + Math.cos(ma) * mtr,
        hp.y + hp.h / 2 + 2,
        hp.z + Math.sin(ma) * mtr,
      );
      root.add(miniTower);
      var miniCrystal = new THREE.Mesh(
        new THREE.OctahedronGeometry(0.3, 0),
        new THREE.MeshBasicMaterial({
          color: [0x00e676, 0x42a5f5, 0xffca28, 0xff6d00, 0x00bcd4, 0xce93d8][
            hi
          ],
          transparent: true,
          opacity: 0.8,
      ...
      root.add(miniCrystal);
      s7_animObjs.push(miniCrystal);
    }

    // Point light per hex platform
    var hexLight = new THREE.PointLight(
      [0x00e676, 0x42a5f5, 0xffca28, 0xff6d00, 0x00bcd4, 0xce93d8][hi],
      0.6,
      25,
    );
  ...
  streamPositions.forEach(function (sp2, si2) {
    for (var sd = 0; sd < 12; sd++) {
      var dot = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({
          color: [0x00e676, 0x42a5f5, 0xffca28][si2 % 3],
          transparent: true,
...
function _s7_findFreeSlot(key) {
  ...
  for (var sp = 1; sp < 30; sp++) {
    var ang = sp * 2.4,
      dist2 = Math.min(sp * 1.5, S7_ISLAND_RADIUS - radius - 1);
...
function _s7_placeItem(itemKey) {
  ...
  loadGLB(itemKey + "_s7", item.glb, function (model) {
    ...
    var terrainY = -9.48; // top surface of grass cylinder (terrain_y=-10, grass y=+0.25, top=+0.52 → world y=-9.48)
    ...
    model.userData.s7Float = 0.1 + Math.random() * 0.08; // subtle float, not too much
    ...
    _s7_ecoBot(
      item.icon + " " + item.label + " berhasil ditanam! " + item.desc,
    );
    _s7_saveToStorage();
  });
}
...
function _s7_showAreaPenuhPopup(item) {
  ...
  if (card) {
    ...
    requestAnimationFrame(function () {
      card.style.animation =
        "s7fullCardIn 0.45s cubic-bezier(0.22,1,0.36,1) forwards, s7fullShake 0.5s ease 0.45s";
    });
  }
  ...
  pop._autoClose = setTimeout(function () {
    _s7_closeAreaFull();
  }, 5000);
}
  ...
  function tick(now) {
    ...
    s7_animObjs.forEach(function (obj) {
      ...
      if (obj.userData.caustic) {
        obj.material.opacity =
          0.15 + Math.sin(t * 2.5 + obj.userData.causticPhase) * 0.12;
      ...
      if (obj.userData.isStream) {
        ...
        obj.material.opacity =
          0.3 + Math.sin(t * 3 + obj.userData.streamPhase) * 0.35;
      ...
      if (obj.userData.cloudBase) {
        ...
        obj.position.y =
          obj.userData.cloudBase.y +
          Math.sin(t * 0.35 + obj.userData.cloudPhase) * 0.4;
        return;
      }
      // GLB rise
      if (obj.userData.s7Rising) {
        ...
        if (Math.abs(obj.position.y - obj.userData.s7TargetY) < 0.02) {
          obj.position.y = obj.userData.s7TargetY;
          obj.userData.s7Rising = false;
        }
        return;
      }
      // GLB float
      if (obj.userData.s7Float !== undefined && !obj.userData.s7Rising) {
        ...
        obj.position.y =
          by +
          Math.sin(t * obj.userData.s7Float + obj.userData.s7FloatPhase) * 0.07;
      }
    });
    ...
    for (var j = 0; j < s7_particles7.length; j++) {
      ...
      if (pt.userData.shockwave) {
        ...
        if (pt.userData.swLife <= 0) {
          scene.remove(pt);
          s7_particles7.splice(j, 1);
          j--;
        }
        continue;
      }
...
function _s7_updateBalance() {
  ...
  s7_placedList.forEach(function (p) {
    for (var i = 0; i < S7_ITEMS.length; i++) {
      if (S7_ITEMS[i].key === p.key) {
        counts[S7_ITEMS[i].category] = (counts[S7_ITEMS[i].category] || 0) + 1;
        break;
      }
    }
  });
  ...
  _s7_setMeter("flora", floraS);
  _s7_setMeter("fauna", faunaS);
  _s7_setMeter("energy", enrgS);
  _s7_setMeter("air", airS, "#29b6f6");
  _s7_setMeter("urban", urbS, "#558b2f");
  var avg = (floraS + faunaS + enrgS + airS + urbS) / 5;
  var st = document.getElementById("s7-balance-status");
  if (!st) return;
  if (s7_placedList.length === 0) {
    st.textContent = "🌱 Tambah komponen!";
    st.style.color = "#888";
  } else if (avg >= 70) {
    st.textContent = "✅ Ekosistem Seimbang!";
    st.style.color = "#00e676";
    st.style.textShadow = "0 0 12px #00e67655";
  } else if (avg >= 40) {
    st.textContent = "⚠️ Hampir Seimbang";
    st.style.color = "#ffca28";
  ...
  ["flora", "fauna", "energy"].forEach(function (id, ii) {
    ...
    var l = document.getElementById(id + "-val");
    if (l) l.textContent = v + "%";
  });
  var bst = document.getElementById("balance-status");
  if (bst)
    bst.textContent =
      avg >= 70 ? "✅ Seimbang!" : avg >= 40 ? "⚠️ Hampir" : "❌ Belum";
  builtObjects.tree = counts.flora;
  builtObjects.animal = counts.fauna;
  builtObjects.solar = counts.energi;
  builtObjects.water = counts.air;
  builtObjects.building = counts.urban;
  try {
...
function _s7_updateXpDisplay() {
  var x = document.getElementById("s7-xp-val");
  if (x) x.textContent = s7_totalXP;
  var c = document.getElementById("s7-count-val");
  if (c) c.textContent = s7_placedList.length;
}

// ============================================================
// STORAGE — simpan data lengkap untuk scene-8
// ============================================================
function _s7_saveToStorage() {
  try {
    // Simpan DETAIL LENGKAP: key, posisi, rotasi — supaya scene-8 bisa replicate persis
    var fullData = s7_placedList.map(function (p) {
      return {
        key: p.key,
        x: p.slot.x,
        z: p.slot.z,
        rotY: p.model ? p.model.rotation.y : 0,
      };
    });
    localStorage.setItem("ar_s7_placed_full", JSON.stringify(fullData));
    // Juga simpan simple array keys untuk kompatibilitas
    localStorage.setItem(
      "ar_s7_placed",
      JSON.stringify(
        s7_placedList.map(function (p) {
          return p.key;
        }),
      ),
    );
...
function _s7_ecoBot(msg) {
  var el = document.getElementById("s7-bot-text");
  if (!el) return;
...
function _s7_injectUI() {
  ...
  S7_ITEMS.forEach(function (it) {
    phtml +=
      '<div class="s7-item-btn" onclick="s7_addItem(\'' +
      it.key +
      '\')" style="--s7c:' +
      it.color +
      ';">' +
      '<div class="s7-item-icon" style="background:' +
      it.color +
      '18;">' +
      it.icon +
      "</div>" +
      '<div class="s7-item-info">' +
      '<div class="s7-item-name">' +
      it.label +
      "</div>" +
      '<div class="s7-item-xp" style="color:' +
      it.color +
      '">+' +
      it.xp +
      " XP</div>" +
  ...
  phtml +=
    '<div class="s7-reset-btn" onclick="s7_confirmReset()">🗑️ Reset Desain</div>';
  ...
  mlist.forEach(function (m) {
    mhtml +=
      '<div class="s7-m-row"><div class="s7-m-label-row"><span class="s7-m-name">' +
      m.label +
      '</span><span class="s7-m-pct" id="s7m-' +
      m.id +
      '-val">0%</span></div>' +
      '<div class="s7-m-track"><div class="s7-m-fill" id="s7m-' +
      m.id +
      '-fill" style="width:0%;background:' +
      m.color +
      '"></div></div></div>';
  });
  mhtml +=
    '<div id="s7-balance-status">🌱 Tambah komponen!</div>' +
    '<button class="s7-next-btn" onclick="s7_goNext()">Lanjut Evaluasi ›</button>';
  meter.innerHTML = mhtml;
  ...
  modal.addEventListener("click", function (e) {
    if (e.target === modal) s7_closeConfirm();
  });
  ...
  fullPop.innerHTML =
    ...
    "</div>";
  fullPop.addEventListener("click", function (e) {
    if (e.target === fullPop) _s7_closeAreaFull();
  });
  document.body.appendChild(fullPop);

  _s7_ecoBot(
    "Selamat datang di Mode Builder! 🏗️ Pilih komponen — setiap objek akan muncul di zona yang tepat. Tidak boleh saling tumpuk!",
  );
  _s7_updateXpDisplay();
}
...
// PUBLIC API
// ============================================================
window.s7_addItem = function (key) {
  _s7_placeItem(key);
};
window.s7_confirmReset = function () {
  document.getElementById("s7-confirm-modal").classList.add("open");
};
window.s7_closeConfirm = function () {
  document.getElementById("s7-confirm-modal").classList.remove("open");
};
window.s7_doReset = function () {
  s7_closeConfirm();
  s7_placedList.forEach(function (p) {
    if (p.model && p.model.parent) scene.remove(p.model);
  });
  s7_placedList = [];
  s7_slotIdx = {};
  s7_occupiedPos = [];
...
window.s7_goNext = function () {
  ...
  flash.style.cssText =
    "position:fixed;inset:0;z-index:9999;background:#00e676;opacity:0;pointer-events:none;transition:opacity 0.25s ease;";
